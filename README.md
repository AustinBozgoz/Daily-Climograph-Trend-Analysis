...
HIstogram value at percentile: An unused function within the daily_climograph_trend_analysis model will take the theoretical histogram inputs (avg,std,skew of the extrapolated normal distribution) and a random number between 0 and 1 to return the value of the histogram associated with that percentile. This has applications in monte-carlo simulations and has been applied to other projects that utilize the rest of this script.
